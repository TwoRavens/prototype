...
errorToPayload <- function(error) {
  return(list(sucess = FALSE, errorMessage = toString(error), errorType = class(error)[1]))
}

postError <- function(error, url) {
  logerror(error, logger = 'runtime')
  res <- POST(url,
              add_headers("Lambda-Runtime-Function-Error-Type" = "Unhandled"),
              body = errorToPayload(error),
              encode = "json")
  logdebug("Posted result:\n%s", toString(res), logger = 'runtime')
}

getSourceFileName <- function(file_base_name) {
...
initializeRuntime <- function() {
  ...
  file_name <- getSourceFileName(file_base_name)
...
# The date is the OpenAPI specification for the runtime api version
...
postResult <- function(result, REQUEST_ID) {
  ...
  body <- toJSON(list(success = TRUE, data = result), auto_unbox = TRUE)
  res <- POST(url, body = body, encode = "raw", content_type_json())
  logdebug("Posted result:\n%s", toString(res), logger = 'runtime')
}

handleRequest <- function(function_name) {
  event_url <- paste0(API_ENDPOINT, "invocation/next")
  event_response <- GET(event_url)
  REQUEST_ID <- event_response$
    headers$
    `Lambda-Runtime-Aws-Request-Id`
  tryCatch({
    EVENT_DATA <- rawToChar(event_response$content)
    result <- invokeLambda(EVENT_DATA, function_name)
