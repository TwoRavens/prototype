...
import { asyncHandler, AUTHENTICATE, callbackHandler, TR_MONGO_DB_NAME, TR_S3_BUCKET_NAME, UserError } from "../constants";
...
export function register_dataset_views(app: Router) {
    ...
    app.post('/dataset/new-file', AUTHENTICATE,
        asyncHandler(async function (req, res, next) {
            multer({
                ...
                await uploadToS3({
                    bucket: TR_S3_BUCKET_NAME,
                    key: datasetId.toHexString()
                }, req.file);

                // ingest into mongo from s3
                await runS3Import({
                    s3: {
                        bucket: TR_S3_BUCKET_NAME,
                        key: datasetId.toHexString()
    ...
    app.post('/dataset/profile', asyncHandler(async function (req, res) {
        let { query, collection_name } = req.body;
