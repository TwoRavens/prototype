...
import {panelMargin} from "../components/common";
import ButtonRadio from '../components/views/ButtonRadio';
import PlotContinuous from './plots/PlotContinuous';
    ...
    view(vnode) {
        ...
        let rightMenu = [
            ...
            mode === 'aggregate' && m('#dateAggreg', m(ButtonRadio, {
                ...
                sections: [
                    process.env.CLIENTSIDE_DOMAIN !== "eventdata" && {value: 'Weekly'},
                    {value: 'Monthly'},
                    process.env.CLIENTSIDE_DOMAIN !== "eventdata" && {value: 'Quarterly'},
                    {value: 'Yearly'},
                ].filter(_=>_)
            }))
        ];
