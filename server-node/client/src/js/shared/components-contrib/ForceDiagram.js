...
export default class ForceDiagram {
    oninit(vnode) {
        ...
        this.kGroupGravity = vnode.attrs.kGroupGravity ?? 4;
    ...
    onupdate({attrs, dom}) {
         ...
         */
        let uppersize = 7;
        let pebbleForceScale = attrs.pebbleForceScale ?? 1;
        function getPebbleCharge(d) {
            ...
            if (groupSize === -Infinity) return -800 * pebbleForceScale;
            if (d.name === attrs.selectedPebble) return -1000 * pebbleForceScale;

            // decrease charge as pebbles become smaller, so they can pack together
            return -400 * pebbleForceScale * (groupSize > uppersize ? uppersize / groupSize : 1);
        }
...
export let groupLinkBuilder = (attrs, context) => {
    [
        ...
        newGroupLinkDefs
            .append("path")
            .attr('d', path)
            .style("fill", groupLink => utils.hexToRgba(groupLink.color, groupLink.opacity ?? 1));
    ...
    context.selectors.groupLinks = context.selectors.groupLinks.enter()
        .append("line")
        .style('fill', 'none')
        .style('stroke', groupLink => utils.hexToRgba(groupLink.color, groupLink.opacity ?? 1))
        .style('stroke-width', groupLink => (groupLink.opacity ?? 1) * 5)
        .on('mousedown', attrs.onclickGroupLink ?? (_=>_))
