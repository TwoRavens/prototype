...
import TextFieldSuggestion from "../components/views/TextFieldSuggestion";
...
import Popper from '../components/views/Popper';
import * as tworavens from "../../tworavens/tworavens";
import {getSelectedProblem, getTargetVariables} from "../../tworavens/problem";
import {setDeep} from "../utils";
import {mapStyles} from "./PlotMapbox";
import * as common from "../components/common";
import Subpanel from "../components/views/Subpanel";
...
export default class PlotVegaLiteEditor {
    ...
    layerEditor(configuration, variables, mapping, summaries, setSummaryAttr, categoricals, abstractQuery, defaultColor) {
        ...
        if (mapping && configuration.mark === 'region' && unusedChannels.includes('region')) {
            let regionUnits = Object.keys(tworavens.locationUnits).filter(unit => !['latitude', 'longitude'].includes(unit));
        ...
        if (configuration.mark === "region" && channel.name === 'region') return [
            ...
            m('',
                ...
                m(Dropdown, {
                    ...
                    items: Object.keys(tworavens.locationUnits).filter(unit => !['latitude', 'longitude'].includes(unit)),
                    ...
                    onclickChild: value => {
                        ...
                        tworavens.inferLocationFormat(channel.variable)
                    }
                }),
                ...
                m('label', 'Format:'),
                    m(Dropdown, {
                        ...
                        items: tworavens.locationUnits[summaries[channel.variable].locationUnit],
                        activeItem: summaries[channel.variable].locationFormat,
                        onclickChild: value => tworavens.setVariableSummaryAttr(channel.variable, 'locationFormat', value)
                    }),
                ),
            ),
            m('div', {onclick: () => channel.delete = true}, m(Icon, {name: 'x'}))
