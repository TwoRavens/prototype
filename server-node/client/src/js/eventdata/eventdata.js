...
export function setSelectedMode(mode) {
    ...
    m.route.set('/eventdata/' + mode.toLowerCase());
...
export let loadMenu = async (abstractPipeline, menu, {recount, requireMatch}={}) => { // the dict is for optional named arguments
    ...
    if (process.env.CLIENTSIDE_DOMAIN === "eventdata" && menu.type !== 'aggregate') {
        // metadata request
        ...
        if (Object.keys(metadata).length)
            promises.push(Auth.post('/metadata/get', metadata).then(setMetadata));
    }
