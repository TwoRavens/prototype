...
import * as queryAbstract from '../shared/manipulations/queryAbstract';
import * as queryMongo from '../shared/manipulations/queryMongo';

import * as common from '../shared/components/common';
import {setLightTheme} from '../shared/components/common';
...
import CanvasDiscrete from "../shared/canvases/CanvasDiscrete";
import CanvasDyad from "../shared/canvases/CanvasDyad";
import CanvasDate from "../shared/canvases/CanvasDate";
import CanvasDiscreteGrouped from "../shared/canvases/CanvasDiscreteGrouped";
import CanvasCoordinates from "../shared/canvases/CanvasCoordinates";
import CanvasCustom from "./canvases/CanvasCustom";
import CanvasResults from "./canvases/CanvasResults";
import CanvasContinuous from "../shared/canvases/CanvasContinuous";

import SaveQuery from "./SaveQuery";
import {TreeAggregate, TreeSubset, TreeVariables} from "../shared/components-contrib/QueryTrees";
import Icon from "../shared/components/views/Icon";
import {italicize} from "../shared/utils";
import {Auth} from "../shared/account";

import twoRavensImage from '../../img/TwoRavens.png';

export default class EventDataPage {

    oninit(vnode) {
        setLightTheme()
        if (vnode.attrs.mode !== 'home') {
            m.route.set('/eventdata/home');
        ...
        // no specific dataset passed in body, so it returns all
        Auth.post('/metadata/get', {'collections': null}).then(eventdata.setMetadata).catch(eventdata.laddaStopAll);
    }
    ...
    header() {
        ...
        return m(Header, {image: '/TwoRavens.png', aboutText: eventdata.aboutText},
            ...
            m('.dropdown[style=float: right; padding-right: 1em]',
                m('#drop.button.btn[type=button][data-toggle=dropdown][aria-haspopup=true][aria-expanded=false]',
                    [Auth.email ?? m('div[style=font-style:oblique]', 'not logged in'), " ", m(Icon, {name: 'triangle-down'})]),
            ...
            app.showModalAlerts && m(ModalVanilla, {
                ...
                app.alerts.length > 0 && m(Table, {
                    data: [...app.alerts].reverse().map(alert => [
                        ...
                        m(`div[style=background:${utils.hexToRgba({
