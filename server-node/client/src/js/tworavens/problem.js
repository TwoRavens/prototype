...
import * as results from "./workspace/results/index";
...
export let buildEmptyProblem = problemId => ({
    ...
    groups: [
        {
            id: 0,
            name: 'Predictors',
            color: tworavens.colors.predictor,
            opacity: 0.3,
            nodes: [],
        ...
        },
        {
            id: 1,
            name: 'Targets',
            color: tworavens.colors.target,
            opacity: 0.3,
            nodes: [],
            description: defaultGroupDescriptions.targets
        }
    ],
    groupLinks: [{source: 0, target: 1, color: tworavens.colors.predictor}],
    description: '',
    metric: undefined,
    metrics: [],
    task: 'classification',
    ...
    splitOptions: {
        ...
        splitsDir: undefined,
        maxRecordCount: tworavens.defaultMaxRecordCount
    },
    searchOptions: {
        timeBoundSearch: 15,
...
export let buildDefaultProblem = problemDoc => {
    ...
    // UTILITY FUNCTIONS
    let findTask = keywords => Object.keys(tworavens.d3mTaskType)
        .find(key => keywords.includes(key)) || 'regression';
    let findSupervision = keywords => Object.keys(tworavens.d3mSupervision)
        .find(key => keywords.includes(key));

    let findSubtask = keywords => {
        let task = findTask(keywords);
        let subTask = keywords.find(keyword => Object.keys(tworavens.applicableMetrics[task]).includes(keyword))
        if (!subTask) subTask = Object.keys(tworavens.applicableMetrics[task])[0];
        return subTask;
    };

    let filterResourceType = keywords => Object.keys(tworavens.d3mResourceType)
        .filter(key => keywords.includes(key));
    let filterD3MTags = keywords => Object.keys(tworavens.d3mResourceType)
        .filter(key => keywords.includes(key));

    // extract a list of tagged variables from the datasetDoc's roles
    let getTagsByRole = role => tworavens.swandive ? [] : tworavens.workspace.datasetDoc.dataResources
    ...
    let predictors = tworavens.swandive
        ? Object.keys(tworavens.variableSummaries)
            .filter(column => column !== 'd3mIndex' && !targets.includes(column))
        : tworavens.workspace.datasetDoc.dataResources // if swandive false, then datadoc has column labeling
            .filter(resource => resource.resType === 'table')
            .flatMap(resource => resource.columns
    ...
    if (predictors.length === 0) {
        predictors = Object.keys(tworavens.variableSummaries).filter(v => !targets.includes(v) && !indexes.includes(v));
    }
    ...
    let problem = {
        ...
        manipulations: [],
        tags: {
            categorical: tworavens.swandive ? [] : tworavens.workspace.datasetDoc.dataResources
                .filter(resource => resource.resType === 'table')
                .flatMap(resource => resource.columns
        ...
        timeGranularity: tworavens.workspace.datasetDoc.dataResources
    ...
    if (problemDoc.statements) {
        ...
        problemDoc.statements.forEach(statement => {
            let source = Object.values(tworavens.variableSummaries)
                .find(summary => summary.worldModelers?.full_concept_name === statement.subj.db_refs.concept).variableName;
            let target = Object.values(tworavens.variableSummaries)
            ...
            link.color = link.sign === "plus" ? tworavens.colors.location : tworavens.colors.boundary;
        ...
        let groups = Object.values(tworavens.variableSummaries)
...
export let erase = problem => {
    ...
    if ('d3mIndex' in tworavens.variableSummaries)
        problem.tags.indexes = ['d3mIndex'];
};

export let generateProblemID = () => 'problem ' + tworavens.workspace.ravenConfig.problemCount++;
...
export function standardizeDiscovery(problems) {
    ...
    //
    problems = problems.filter(problem => problem.targets && problem.targets.every(target => target in tworavens.variableSummaries));
    ...
    return problems.reduce((out, prob) => {
        ...
        // if (process.env.CLIENTSIDE_DOMAIN !== "d3m") {
        ...
        // console.log('variableSummaries:' + JSON.stringify(tworavens.variableSummaries))
        ...
        let ordering = predictors.filter(variable => tworavens.variableSummaries[variable]?.timeUnit);
        ...
        setTask(
            tworavens.inferIsCategorical(tworavens.variableSummaries[prob.targets[0]])
...
export let setTask = (task, problem) => {
    ...
    if (task.toLowerCase() === 'classification')
        setSubTask(tworavens.variableSummaries[targets[0]].binary ? 'binary' : 'multiClass', problem);
    else if (task.toLowerCase() === 'regression')
        setSubTask(targets.length > 1 ? 'multivariate' : 'univariate', problem);
    else if (!(problem.subTask in tworavens.applicableMetrics[task]))
        setSubTask(Object.keys(tworavens.applicableMetrics[task])[0], problem);
...
export let setSubTask = (subTask, problem) => {
    if (subTask === problem.subTask || !Object.keys(tworavens.applicableMetrics[problem.task]).includes(subTask))
        return;
    problem.subTask = subTask;
    setMetric(tworavens.applicableMetrics[problem.task][getSubtask(problem)][0], problem, true);
...
export let setSupervision = (supervision, problem) => {
    if (supervision === problem.supervision || ![undefined, ...Object.keys(tworavens.d3mSupervision)].includes(supervision))
        return;
...
export let setResourceTypes = (types, problem) => {
    types = types.filter(type => Object.keys(tworavens.d3mResourceType).includes(type));
...
export let setD3MTags = (tags, problem) => {
    tags = tags.filter(type => Object.keys(tworavens.d3mTags).includes(type));
...
export let getSubtask = problem => {
    ...
    if (!problem.subTask && tworavens.variableSummaries[targets[0]]) {
        if (problem.task.toLowerCase() === 'classification')
            problem.subTask = tworavens.variableSummaries[targets[0]].binary ? 'binary' : 'multiClass';
        else if (problem.task.toLowerCase() === 'regression')
            problem.subTask = targets.length > 1 ? 'multivariate' : 'univariate';
        else
            problem.subTask = Object.keys(tworavens.applicableMetrics[problem.task])[0]
    } else if (!problem.subTask && !tworavens.variableSummaries[targets[0]])
        return Object.keys(tworavens.applicableMetrics[problem.task])[0];
...
export let setMetric = (metric, problem, all = false) => {
    if (metric === problem.metric || !tworavens.applicableMetrics[problem.task][getSubtask(problem)].includes(metric))
        return;
    ...
    if (all) problem.metrics = tworavens.applicableMetrics[problem.task][getSubtask(problem)]
...
export let getCategoricalVariables = problem => {
    ...
    return [...new Set([
        ...Object.keys(tworavens.variableSummaries).filter(variable => tworavens.variableSummaries[variable].nature === "nominal"),
...
export let getOrdinalVariables = problem => {
    let selectedProblem = problem || getSelectedProblem();
    return [...new Set([
        // ...Object.keys(tworavens.variableSummaries).filter(variable => tworavens.variableSummaries[variable].nature === "ordinal"),
...
export let getOrderingTimeUnit = problem => {
    let units = problem.tags.ordering
        .map(variable => tworavens.variableSummaries[variable]?.timeUnit);
...
export let getGeographicVariables = () =>
    Object.keys(tworavens.variableSummaries).filter(variable => tworavens.variableSummaries[variable].locationUnit);
 ...
 * @returns {(string | undefined)}
 */
export let getAbstractPipeline = (problem, all=undefined) => {
    if (!problem) return [...tworavens.workspace.ravenConfig.hardManipulations]
    ...
    return [
        ...
        {
            type: 'menu',
            metadata: !all && (modelVariables.length < Object.keys(tworavens.variableSummaries).length) ? {
                type: 'data',
...
export let loadProblemPreprocess = async problem =>
    tworavens.loadPreprocess(JSON.stringify(queryMongo.buildPipeline([
        ...getAbstractPipeline(problem, true),
        {type: 'menu', metadata: {type: 'data', sample: tworavens.preprocessSampleSize}}
    ], tworavens.workspace.ravenConfig.variablesInitial)['pipeline']));
...
export let isProblemValid = problem => {
    ...
    if (problem.task !== "clustering") {
        ...
        if (problem.task === "classification" && (tworavens.variableSummaries[targets?.[0]]?.uniqueCount ?? 0) > 100) {
...
export let needsManipulationRewritePriorToSolve = problem => {
    ...
    Object.keys(tworavens.variableSummaries)
        .filter(variable => tworavens.variableSummaries[variable].numchar === 'character')
    ...
    let hasManipulation = (tworavens.workspace.ravenConfig.hardManipulations.length + problem.manipulations.length) > 0;
...
export let getProblemTrees = () => {
    ...
    let problems = Object.values(workspace.ravenConfig.problems)
