...
import * as queryMongo from "../../../shared/manipulations/queryMongo";
...
import Slider from "../../../shared/components/views/slider";

import PlotVegaLite from "../../../shared/components-contrib/PlotVegaLite";
import ConfusionMatrix from "../../../shared/components-contrib/ConfusionMatrix";
import Flowchart from "../../../shared/components-contrib/Flowchart";
import ModelInterpretation from "../../../shared/components-contrib/ModelInterpretation";
...
import {preparePanels} from "../../../shared/components-contrib/PlotVegaLiteWrapper";
import {ExploreBoxes, explorePreferences, ExploreVariables} from "../explore/index";
import Checkbox from "../../../shared/components/views/Checkbox";
        ...
        sections: [
            {
                ...
                contents: [
                    ...
                    m(Table, {
                        data: Object.keys(tworavens.workspace.ravenConfig.problems)

                            .map(problemId => tworavens.workspace.ravenConfig.problems[problemId])
                        ...
                        activeRow: tworavens.workspace.ravenConfig.selectedProblem,
...
export class CanvasSolutions {
    ...
    oninit() {
        ...
        tworavens.updateRightPanelWidth()
    }
                ...
                specification: {
                    ...
                    "encoding": {
                        ...
                        "y": {
                            ...
                            scale: (metric in tworavens.d3mMetricDomains)
                                ? {domain: tworavens.d3mMetricDomains[metric]}
                                : {}
                        },
                        "tooltip": [
                        ...
                        makePage: variablesToPlot => variablesToPlot
                            ...
                            .map(predictor => [
                                ...
                                m(ModelInterpretation, {
                                    ...
                                    summary: tworavens.variableSummaries[predictor],
                ...
                m('div', {style: 'overflow:auto'}, m(Paginated, {
                    ...
                    makePage: data => data.map(predictorEntry => m('div',
                        ...
                        m(ModelInterpretation, {
                            ...
                            yLabel: valueLabel,
                            variableLabel: variableLabel,
                            summary: tworavens.variableSummaries[predictorEntry.predictor],
                            levels: resultsCache[problem.problemId].levels
                        }))),
                    limit: 10,
                    page: resultsPreferences.interpretationPage,
    ...
    variableExplore(problem, adapters) {
            ...
            foldedVariables.reduce((out, variable) =>
                Object.assign(out, {
                    [variable]: Object.assign({},
                        tworavens.variableSummaries[resultsPreferences.target] || {},
                        {variableName: variable, name: variable})
                }), {}),
            // problem.results.variablesInitial.reduce((out, variable) =>
            //     Object.assign(out, {[variable]: tworavens.variableSummaries[variable] || {}}), {}),
            tworavens.variableSummaries);
        ...
        if (!splitPath) return;
        let splitCollectionName = `${tworavens.workspace.name}_split_${utils.generateID(splitPath)}`;
    ...
    customExplore(problem, adapters, mapping) {
        ...
        ].reduce((out, variable) => Object.assign(out, {[variable]: tworavens.variableSummaries[variable] || {}}), {});
        ...
        let {editor, plot} = preparePanels({
            ...
            setSummaryAttr: (variable, attr, value) => {
                if (variable in tworavens.variableSummaries)
                    tworavens.setVariableSummaryAttr(variable, attr, value)
                else {
                    utils.setDeep(resultsPreferences, ['variableSummariesDiffs', variable, attr], value);
                    utils.setDeep(tworavens.variableSummaries, [variable, attr], value);
                }
            },
        ...
        let solutionSummary = selectedAdapters.length === 1 && m(Subpanel, {
            ...
            m(Table, {
                data: [
                    ...
                    ['Model Zip', m(Button, {
                        onclick: () => {
                            solvers.downloadModel(firstSolution)
                        }
                    }, 'Download')]
                ]),
            ...
            m(Table, {
                ...
                data: Object.keys(resultsCache[problem.problemId]?.producePaths[firstSolution.solutionId] ?? {})
                    .map(produceName => ({
                        ...
                        'input': m(Button, {
                            onclick: () => tworavens.downloadFile(
...
export let getSolutionAdapter = (problem, solution) => ({
    ...
    getScore: metric => {
        ...
        let evaluation = solution.scores.find(score => tworavens.d3mMetricsInverted[score.metric.metric] === metric);
...
let getSystemAdapters = problem => {
    ...
    let solverCandidateNames = tworavens.applicableSolvers[problem.task][getSubtask(problem)];

    // only show solvers that are capable of solving this type of problem
    let solverSystemNames = process.env.CLIENTSIDE_TA2_WRAPPED_SOLVERS // set in templates/index.html
        .split(',').filter(name => solverCandidateNames.includes(name));

    return solverSystemNames
        .reduce((out, systemId) => Object.assign(out, {
            [systemId]: solvers.getSystemAdapterWrapped(systemId, problem)
        }), {});
}
 ...
 * @returns {*}
 */
let getComparableProblems = selectedProblem => Object.values(tworavens.workspace.ravenConfig.problems)
...
let getSolutionTable = (problems, systemId) => {
    ...
    let data = adapters
        ...
        .map(adapter => Object.assign(
            ...
            [resultsPreferences.selectedMetric || problem.metric]
                .reduce((out, metric) => Object.assign(out, {
                    [metric]: utils.formatPrecision(adapter.getScore(metric))
                }), {})))
    let allMetrics = [problem.metric, ...problem.metrics];
    return [
        data.length > 0 && allMetrics.length > 1 && m(ButtonRadio, {
            id: 'selectedMetricButtonRadio',
            attrsAll: {style: {'margin-bottom': '.5em', width: 'calc(100% - 1em)', 'margin-left': '.5em'}},
            onclick: metric => resultsPreferences.selectedMetric = metric,
            activeSection: resultsPreferences.selectedMetric || selectedProblem.metric,
            sections: [problem.metric, ...problem.metrics].map(value => ({value}))
        }),
        m('div[style=width:100%;overflow:auto]',
            m(Table, {
                id: 'solutionTable' + (systemId || ''),
                data,
...
let getResultsAbstractPipelineTargets = (problem, adapters) => adapters
    ...
    .map(([adapter, producePath]) => ({
        type: 'join',
        from: `${tworavens.workspace.name}_split_${utils.generateID(problem.results.datasetPaths[resultsPreferences.dataSplit])}_produce_${utils.generateID(producePath)}`,
...
let getResultsAbstractPipeline = (problem, splitCollectionName, splitDatafile, all) => {
    ...
    if (all || resultsQuery[0].abstractQuery.some(node => queryMongo.getSubsetDependencies(node).length > 0)) {
        return {
            collectionName: tworavens.workspace.name,
            datafile: tworavens.workspace.datasetPath,
            pipeline: [
                ...getAbstractPipeline(problem, true),
                {
                    type: 'join',
            ...
            variablesInitial: tworavens.workspace.ravenConfig.variablesInitial,
...
export let loadFittedVsActuals = async (problem, adapter) => {
    ...
    let response;
    try {
        response = await m.request(process.env.D3M_SVC_URL + `/retrieve-output-fitted-vs-actuals-data`, {
            method: 'POST',
            body: {
...
export let loadConfusionData = async (problem, adapter) => {
    ...
    let response;
    try {
        response = await m.request(process.env.D3M_SVC_URL + `/retrieve-output-confusion-data`, {
            method: 'POST',
            body: {
                data_pointer: producePointer,
                metadata: {
...
export let loadFittedData = async (problem, adapter, split) => {
    ...
    let response;
    try {
        response = await m.request(process.env.D3M_SVC_URL + `/retrieve-output-data`, {
            method: 'POST',
            body: {
                data_pointer: producePointer,
    ...
    resultsCache[problem.problemId].interpretationPartialsFitted[adapter.getSolutionId()] =
        ...
        out[predictor] = response.data.slice(offset, nextOffset)
            ...
            .map(point => getTargetVariables(problem).reduce((out_point, target) => Object.assign(out_point, {
                [target]: tworavens.inferIsCategorical(target) ? point[target] : utils.parseNumeric(point[target])
            }), {}));
        offset = nextOffset;
        return out;
    }, {});
...
export let loadInterpretationEFDData = async (problem, adapter) => {
    ...
    try {
        response = await m.request(process.env.D3M_SVC_URL + `/retrieve-output-EFD-data`, {
            method: 'POST',
            body: {
                ...
                metadata: {
                    ...
                    collection_name: `${tworavens.workspace.name}_split_${utils.generateID(splitPath)}`, // collection/dataset name
                    query: compiled
                }
            }
        });
...
export let loadInterpretationICEFittedData = async (adapter, predictor) => {
    ...
    let response;
    try {
        response = await m.request(process.env.D3M_SVC_URL + `/retrieve-output-ICE-data`, {
            method: 'POST',
            body: {
...
let loadImportanceScore = async (problem, adapter, mode) => {
    ...
    //         try {
    //             responses[predictor] = await m.request(process.env.D3M_SVC_URL + `/retrieve-output-ICE-data`, {
    //                 method: 'POST',
    //                 body: {
    ...
    let responseImportance = await m.request(process.env.ROOK_SVC_URL + 'variableImportance.app', {
        method: 'POST',
        body: {
...
let loadPredictorDomains = async problem => {
    ...
    resultsCache[problem.problemId].levels = Object.keys(facets).length > 0 ? (await app.getData({
        ...
        collection_name: `${tworavens.workspace.name}_split_${resultsPreferences.dataSplit}`, // collection/dataset name
    ...
    resultsCache[problem.problemId].domains = predictors.reduce((domains, predictor) => {
        let summary = tworavens.variableSummaries[predictor];
        if (!summary.validCount)
            ...
            if (tworavens.variableSummaries[predictor].binary)
                domains[predictor] = [tworavens.variableSummaries[predictor].min, tworavens.variableSummaries[predictor].max];
            else
                domains[predictor] = utils.linspace(
                    tworavens.variableSummaries[predictor].min,
                    tworavens.variableSummaries[predictor].max,
                    ICE_DOMAIN_MAX_SIZE)
        }
        return domains;
    }, {})
...
let loadICEDatasetPaths = async problem => {
    ...
    try {
        ...
        partialsLocationInfo = await m.request({
            method: 'POST',
            url: process.env.D3M_SVC_URL + '/get-partials-datasets',
            body: {
                // run on this dataset
                ...
                problem: solvers.SPEC_problem(problem),
...
let loadPartialsDatasetPath = async problem => {
    ...
    let dataset = [Object.keys(tworavens.variableSummaries)
        .reduce((record, variable) => Object.assign(record, {
            [variable]: resultsCache[problem.problemId].levels?.[variable]?.[0]?.level
                ?? tworavens.variableSummaries[variable].median // take most frequent level (first mode)
        }), {})];
...
export let getSummaryData = problem => ({
    dataset_name: tworavens.workspace.name,
    problem: problem,
...
export let findProblem = data => {
    ...
    return Object.values(tworavens.workspace?.ravenConfig?.problems || {})
...
export let prepareResultsDatasets = async (problem, solverId) => {
    ...
    utils.setDefault(problem.results, 'd3mDatasetId',
        tworavens.workspace.datasetDoc.about.datasetID + '_' + Math.abs(utils.generateID(String(Math.random()))));
    ...
    m.redraw();
    try {
        if (!tworavens.materializeManipulationsPromise[problem.problemId])
            tworavens.materializeManipulationsPromise[problem.problemId] = tworavens.materializeManipulations(problem);
        await tworavens.materializeManipulationsPromise[problem.problemId];
    } catch (err) {
    ...
    m.redraw();
    try {
        if (!tworavens.materializeTrainTestPromise[problem.problemId])
            tworavens.materializeTrainTestPromise[problem.problemId] = tworavens.materializeTrainTest(problem);
        await tworavens.materializeTrainTestPromise[problem.problemId];
    } catch (err) {
        console.error(err);
    ...
    problem.results.variablesInitial = [...buildPipeline(
        getAbstractPipeline(problem), tworavens.workspace.ravenConfig.variablesInitial
    )['variables']]
...
export let uploadForModelRun = async (file, name, problem) => {
    ...
    let manipulatedInfo = await tworavens.buildDatasetPath(
        problem, undefined, customDataset.datasetPath,
        `${tworavens.workspace.name}_${customDataset.name}`,
