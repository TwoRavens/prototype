...
from api.data import encode_variable, decode_variable
from api.database.connect import MongoClientUtil

HOST_CHOICES = (u'TwoRavens', u'UTDallas')


def run_query(
        database_name: str, collection_name: str,
        query: List[dict],
        method: str, distinct=None,
        comment=None, user=None, host="TwoRavens"):
    ...
    collection: Collection = MongoClientUtil().get_collection(database_name, collection_name)
...
def _run_utdallas_query(collection_name, method, query, distinct=None):
    ...
    url = os.environ.get("UTDALLAS_PRODUCTION_SERVER_ADDRESS") \
