...
import {alignmentData, getData, alertError, alertWarn, MOCKED} from "../app";
...
import {debug, unwrap} from "../shared/utils";

import * as model from "./workspace/model";
import * as results from "./workspace/results";
import * as explore from "./workspace/explore";
 ...
 * @member {string} name
 * @member {RavenConfig} ravenConfig
...
export let setWorkspace = value => workspace = value;
    ...
    if (isResultsMode) {
        ...
        if (splitPath) {
            datasetDetails = {
                ...
                collection_name: `${workspace.name}_split_${utils.generateID(splitPath)}` // collection/dataset name
            }
...
export let downloadFile = async (datasetUrl, contentType) => {
    ...
    let downloadUrl = process.env.D3M_SVC_URL + '/download-file?' + m.buildQueryString(data);
...
export let isDatasetsMode = true;
export let isWorkspacesMode = false;
export let isModelMode = false;
...
export function setSelectedMode(mode) {
    ...
    mode = mode ? mode.toLowerCase() : 'datasets';

    let previousMode = selectedMode;

    let selectedProblem = getSelectedProblem();
    if (!workspace && ['dataset', 'model', 'explore', 'results'].includes(mode)) {
        return;
    }
    ...
    isDatasetsMode = mode === 'datasets';
    isWorkspacesMode = mode === 'workspaces';
    ...
    if (workspace && !isDatasetMode && previousMode === 'dataset') {
        let ravenConfig = workspace.ravenConfig;
    ...
    if (selectedMode !== mode) {
        ...
        if (!isExploreMode && previousMode === 'explore' && selectedProblem?.system === 'discovered') {
            ...
            workspace.ravenConfig.problems[copiedProblem.problemId] = copiedProblem;
        ...
        if (!isDatasetMode && manipulate.pendingHardManipulation) {
            ...
            let datasetQuery = JSON.stringify(queryMongo.buildPipeline([
                ...workspace.ravenConfig.hardManipulations,
                {type: 'menu', metadata: {type: 'data'}}
            ], workspace.ravenConfig.variablesInitial)['pipeline']);
...
export async function buildCsvPath(problem, lastStep, dataPath, collectionName) {
    ...
    let compiled = queryMongo.buildPipeline(abstractPipeline, workspace.ravenConfig.variablesInitial)['pipeline'];
    let body = {
        method: 'aggregate',
...
export let saveLogEntry = async logData => {
    ...
    Auth.post(save_log_entry_url, logData)
...
export let setLeftTab = (tabName) => {
    ...
    if (tabName === LEFT_TAB_NAME_DISCOVER && tutorial_mode)
...
if (process.env.CLIENTSIDE_DOMAIN !== "eventdata") {
...
//
if (process.env.CLIENTSIDE_DOMAIN !== "eventdata") {
    if (MOCKED) document.addEventListener("DOMContentLoaded", connectWebsocket);
}
...
function websocketMessage(e) {
    streamMsgCnt++;
    debug.log(streamMsgCnt + ') message received! ' + e);
    ...
    try {
        ...
        if (msg_data.msg_type === 'receive_describe_msg')
            ...
            debug.log('streamSocket.onmessage: Error, "msg_data.data" type not specified!');
            debug.log('full data: ' + JSON.stringify(msg_data));
            debug.log('---------------------------------------------');
        } else {
...
/**
 ...
 4. Create ravenConfig if undefined
 a. Assign discovered problems into ravenConfig
...
let loadSampledDatasetPath = async query => {
    ...
    if (!samplingCache.sampledDatasetPathPromise) {
        samplingCache.sampledDatasetPathPromise = query.length > 0
            ...
            : Promise.resolve(workspace.ravenConfig.datasetPath)
    }
...
let loadDiscovery = async query => {
    ...
    if (!samplingCache.discoveryPromise)
        samplingCache.discoveryPromise = loadSampledDatasetPath(query)
            .then(datasetPath => m.request(process.env.ROOK_SVC_URL + 'discovery.app', {
                method: 'POST',
...
export let loadPreprocess = async query => {
    ...
    if (!samplingCache.preprocessPromise)
        ...
        // let vars = queryMongo.buildPipeline(query, workspace.variablesInitial)
        // samplingCache.preprocessPromise = Auth.post('/data/query', {
        //     body: {
        //         collection_name: 'baseball',
        //         query: queryMongo.buildPipeline([...query, {
        //             type: 'menu',
        //             metadata: {type: 'summary', variables: [""]}
        //         }])
        //     }
        // })
        samplingCache.preprocessPromise = loadSampledDatasetPath(query)
            .then(datasetPath => Auth.post("/dataset/profile", {
                data: datasetPath,
                datastub: workspace.name,
                query,
...
export let loadWorkspace = async (newWorkspace, awaitPreprocess = false) => {
    ...
    document.title = "TwoRavens " + workspace.name;

    let newRavenConfig = !workspace.ravenConfig;
    console.log(workspace);
    if (newRavenConfig) workspace.ravenConfig = {
    ...
    let datasetQuery = queryMongo.buildPipeline(
        manipulations, workspace.ravenConfig.variablesInitial)['pipeline']

    // ~~~~ BEGIN PROMISE GRAPH ~~~~

    // PREPROCESS
    let promisePreprocess = Auth.post('/dataset/profile', {
        _id: workspace.datasetId,
        query,
    })
        .then(setPreprocess)
        .catch(err => {
            alertError(m('div',
                m('[style=margin-right:1em;display:inline-block]', "Preprocess failed."),
                m(Button, {onclick: () => location.reload()}, "Reload Page")))
            throw err;
        });
    ...
    //         Object.assign(workspace.ravenConfig.problems, standardizeDiscovery(discovery))
    ...
    //         localStorage.setItem('peekHeader' + peekId, "TwoRavens " + workspace.name);
    //     });

    // final cleanup on the selected problem
    // let selectedProblemPromise = promiseDiscovery
    //     .then(() => {
    //         let problem = getSelectedProblem();
    ...
    //             setSelectedProblem(Object.values(workspace.ravenConfig.problems)
    ...
    //             let newPredictors = getPredictorVariables(workspace.ravenConfig.problems['problem 3'])
...
export async function load() {
    if (process.env.CLIENTSIDE_DOMAIN === "eventdata") {
        m.route.set('/eventdata')
        return;
    }

    // Load current workspace
    return await Auth.post('/workspace/get')
        .then(loadWorkspace)
        .catch(err => {
            console.error(err)
...
//         method: 'POST',
//         url: process.env.D3M_SVC_URL + '/get-train-test-split-indices',
//         body: {
...
export let materializeTrainTest = async problem => {
    ...
    let response = await m.request({
        method: 'POST',
        url: process.env.D3M_SVC_URL + '/get-train-test-split',
        body: {
...
export let setVariableSummaries = state => {
    ...
    Object.entries(workspace.ravenConfig.variableSummariesDiffs)
...
export let inferLocationFormat = variable => {
    ...
    Auth.post('/metadata/get', {alignments: units})
...
export let setVariableSummaryAttr = (variable, attr, value) => {
    utils.setDeep(workspace, ['ravenConfig', 'variableSummariesDiffs', variable, attr], value);
    utils.setDeep(variableSummaries, [variable, attr], value);
};

export let setDatasetSummaryAttr = (attr, value) => {
    utils.setDeep(workspace, ['ravenConfig', 'datasetSummaryDiffs', attr], value);
...
export let setDatasetSummary = state => {
    ...
    common.deepMerge(datasetSummary, workspace.ravenConfig.datasetSummariesDiffs);
};
...
export let saveUserWorkspace = (silent = false, reload = false) => {
    ...
    let ravenConfig_save_url = '/user-workspaces/raven-configs/json/save/' + workspace.user_workspace_id;

    console.log('data to save: ' + JSON.stringify(workspace.ravenConfig))

    m.request({
        method: "POST",
        url: ravenConfig_save_url,
        body: {ravenConfig: workspace.ravenConfig}
    })
        .then(function (save_result) {
...
export async function saveAsNewWorkspace() {
    ...
    // save url
    let ravenConfig_save_url = '/user-workspaces/raven-configs/json/save-as-new/' + workspace.user_workspace_id;
    ...
    let save_result = await m.request({
        ...
        body: {
            ...
            ravenConfig: workspace.ravenConfig
        }
    });
...
let getSolutionIds = workspace => Object.values(workspace.ravenConfig.problems)
