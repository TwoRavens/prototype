...
import densityschema from './vega-schemas/univariate/density.json'
...
import PlotVegaLiteWrapper from "../../../shared/components-contrib/PlotVegaLiteWrapper";
import {bold, italicize, remove, toggle} from "../../../shared/utils";
import {
    getAbstractPipeline,
...
let get_node_label = problemOrVariableName => {
    if (explorePreferences.mode === 'problems') {
        let exploreProblem = 'problems' in tworavens.workspace.ravenConfig && tworavens.workspace.ravenConfig.problems[problemOrVariableName];
...
export class ExploreBoxes {
    view(vnode) {
        ...
        let exploreBoxIds = Object.keys(preferences.mode === "problems" ? tworavens.workspace.ravenConfig.problems : summaries);
            ...
            makePage: exploreBoxIds => m('',
                ...
                return m('span#exploreNodeBox', {
                        ...
                        style: {
                            ...
                            width: '250px',
                            'align-items': 'center',
                            'background-color': selected ? utils.hexToRgba(common.colors.selVar) : common.colors.base
                        }
                    },
                    m('#exploreNodePlot', {
                        oninit() {
                        ...
                        onupdate(vnode) {
                            let targetName = preferences.mode === 'problems'
                                ? getTargetVariables(tworavens.workspace.ravenConfig.problems[boxId])[0]
                                : boxId;
    ...
    view(vnode) {
        ...
        if (!explorePreferences.go) return [wrapCanvas(
            ...
            !['custom', 'mapping'].includes(explorePreferences.mode) && m(Button, {
                ...
                onclick: () => {
                    let selected = explorePreferences.mode === 'problems'
                        ? [tworavens.workspace.ravenConfig.selectedProblem]
                    ...
                    m.route.set(`/workspace/explore/${explorePreferences.mode}/${selected.join('/')}`);
                }
            }, 'go'),
            ...
            !['custom', 'mapping'].includes(explorePreferences.mode) && m(ExploreBoxes, {
                    ...
                    summaries: tworavens.variableSummaries,
                })
        ),
            explorePreferences.mode === 'mapping' && m('div', {
                ...
                m(PlotVegaLiteWrapper, {
                    ...
                    summaries: tworavens.variableSummaries,
                    setSummaryAttr: tworavens.setVariableSummaryAttr,
...
export async function loadPlotSpecification(preferences, summaries, abstractQuery, getData) {
    ...
    await Promise.all(facetSummaries.map(async (nodeSummaries, i) => {
        ...
        let {pipeline: compiled, datasets} = queryMongo.buildPipeline(
            ...
            tworavens.workspace.ravenConfig.variablesInitial);
        ...
        let response = await m.request(process.env.ROOK_SVC_URL + 'plotData.app', {method: 'POST', body: jsonout});
        if (!response.success) {
