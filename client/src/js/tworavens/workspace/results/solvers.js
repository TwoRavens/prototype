...
let SPEC_metric = (positiveLabel, metric) => {
    let value = {metric: tworavens.d3mMetrics[metric]};
...
export let SPEC_problem = problem => {
    ...
    return {
        ...
        "taskSubtype": tworavens.d3mTaskSubtype[problem.subTask],
        "forecasting": problem.task === "forecasting",
        "taskType": problem.task === "forecasting" ? "REGRESSION" : tworavens.d3mTaskType[problem.task],
...
let SPEC_configuration = problem => ({
    ...
    "method": tworavens.d3mEvaluationMethods[problem.scoreOptions.evaluationMethod],
...
let endStopWrappedSearch = (problem, systemId) => searchId => {
    ...
    let solvedProblem = Object.values(tworavens.workspace.ravenConfig.problems)
...
export let downloadModel = async solution => m.request(SOLVER_SVC_URL + 'Download', {
    ...
    tworavens.downloadFile(response.data.model_pointer, 'application/octet-stream');
});
